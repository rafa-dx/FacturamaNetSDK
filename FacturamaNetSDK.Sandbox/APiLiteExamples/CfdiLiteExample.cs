...
namespace FacturamaNetSDK.Sandbox.APiLiteExamples
{
    public class CfdiLiteExample

    {
        private readonly FacturamaClient _client;
        private string? _cfdiId; // guardamos el ID para usarlo en los siguientes ejemplos

        public CfdiLiteExample(FacturamaClient client)
        {
            _client = client;
        }
        ...
        {
            Console.WriteLine("\n========================================");
            Console.WriteLine("  CFDI LITE — API Web");
            Console.WriteLine("========================================\n");
            try
            {
                await CreateAsync();
        ...
        private async Task CreateAsync()
        {
            ...
            var request = new CfdiLiteRequest
            {
                NameId = "1",
                Folio = "99",
                Serie = "FAC",
            ...
            var response = await _client.CfdiLite.CreateAsync(request);
            _cfdiId = response.Id;
        ...
        private async Task GetAsync()
        {
            ...
            var response = await _client.CfdiLite.GetAsync(_cfdiId);
        ...
        private async Task ListAsync()
        {
            ...
            var response = await _client.CfdiLite.ListAsync(filters);
        ...
        private async Task DownloadPdfAsync()
        {
            ...
            var response = await _client.CfdiLite.DownloadAsync(CfdiFileType.Pdf, _cfdiId);
        ...
        private async Task DownloadXmlAsync()
        {
            ...
            var response = await _client.CfdiLite.DownloadAsync(CfdiFileType.Xml, _cfdiId);
            Console.WriteLine($"  ContentType: {response.ContentType}");
            Console.WriteLine($"  Tamaño: {response.ContentLength} bytes\n");
        }
        ...
        private async Task SendByEmailAsync()
        {
            ...
            var response = await _client.CfdiLite.SendByEmailAsync(_cfdiId, "prueba@facturama.mx");
        ...
        private async Task CancelAsync()
        {
            ...
            var response = await _client.CfdiLite.CancelAsync(_cfdiId, motive: "02");
            Console.WriteLine($"  Status: {response.Status}");
        ...
        private static readonly JsonSerializerOptions _printOptions = new()
        {
            WriteIndented = true,
        ...
        {
            var json = JsonSerializer.Serialize(obj, _printOptions);
